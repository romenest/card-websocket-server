...
public class WebSocketSessionManager {
    ...
    public void addSession(WebSocketSession session, String roomId, String senderName) {
        ...
        broadcastToRoom(roomId, "system", String.format("[%s]님이 채널 %s에 입장했습니다.", senderName, roomId));
    }
    ...
    public void broadcastToRoom(String roomId, String sender, String message) {
        ...
        try {
            ...
            Mono.when(sessions.values().stream()
                    .filter(info -> info.getRoomId().equals(roomId) && info.getSession().isOpen())
